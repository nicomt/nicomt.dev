...
export function colorTemperature2rgbUsingTH(kelvin: number) {
...
export function colorTemperature2rgb(kelvin: number) {
  ...
  if (temperature < 66.0) {
    ...
    green = temperature - 2;
    green = (
      -155.25485562709179 - 0.44596950469579133
       * green + 104.49216199393888 * Math.log(green)
    );
    if (green < 0) green = 0;
    if (green > 255) green = 255;
  } else {
 ...
 */
export function rgb2colorTemperature(rgb: { red: number; blue: number; green: number }) {
  let temperature; let
    testRGB;
  const epsilon = 0.4;
  ...
  if (!temperature) return Number.NaN;
